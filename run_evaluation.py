...
def main(config: DictConfig):
    ...
    events = jnp.concatenate(events_per_sample)
    time = jnp.concatenate(time_per_sample)
    targets = jnp.concatenate(targets)

    df = pd.DataFrame({'predictions': predictions, 'targets': targets, 'events': events, 'time': time})
    save_path = os.path.join(config.logging.log_dir, 'test_stats.csv')
    print(save_path)
