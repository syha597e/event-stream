...
def custom_loader(cache_dir: str, bsz: int = 50, seed: int = 42) -> Data: ...
...
def event_stream_collate_fn(batch, resolution, max_time=None):
    ...
    if len(resolution) == 1:
        ...
        tokens = [
            torch.tensor(
                e["x"] * e["y"] + np.prod(resolution) * e["p"].astype(np.int32)
            ).int()
            for e in x
        ]
    else:
    ...
    timesteps = torch.stack(
        [pad(e, (0, max_length - len(e)), "constant", e[-1]) for e in timesteps]
    )
    ...
    tokens = torch.stack(
        [pad(e, (0, max_length - len(e)), "constant", -1) for e in tokens]
    )
...
def event_stream_dataloader(
    train_data, val_data, test_data, bsz, collate_fn, rng, shuffle_training=True
):
...
def create_events_shd_classification_dataset(
    ...
    **kwargs,
    ...
    transforms = tonic.transforms.Compose(
        [
            tonic.transforms.TimeJitter(
                std=100, clip_negative=False, sort_timestamps=True
            ),
    ...
    train_data, val_data = torch.utils.data.random_split(
        train_data, lengths=[len(train_data) - val_length, val_length], generator=rng
    )
    ...
    return Data(
        ...
        n_classes=20,
        in_dim=700,
        train_pad_length=16384,
        test_pad_length=16384,
...
def create_events_ssc_classification_dataset(
    ...
    **kwargs,
    ...
    train_data = tonic.datasets.SSC(
        save_to=cache_dir, split="train", transform=transforms
    )
    ...
    return Data(
        ...
        n_classes=35,
        in_dim=700,
        train_pad_length=20480,
        test_pad_length=20480,
        train_size=len(train_data),
    )
...
def create_events_dvs_gesture_classification_dataset(
    ...
    transforms = [
        ...
        tonic.transforms.SpatialJitter(
            sensor_size=(128, 128, 2), var_x=4, var_y=4, clip_outliers=True
        ),
        tonic.transforms.DropEventByArea(
            sensor_size=(128, 128, 2), area_ratio=(0.05, 0.2)
        ),
    ]
    ...
    train_data = tonic.datasets.DVSGesture(
        save_to=cache_dir, train=True, transform=transforms
    )
    val_length = int(0.1 * len(train_data))
    train_data, val_data = torch.utils.data.random_split(
        train_data, lengths=[len(train_data) - val_length, val_length], generator=rng
    ...
    return Data(
        ...
        n_classes=11,
        in_dim=128 * 128 * 2,
        train_pad_length=crop_events,
        test_pad_length=1595392,
        train_size=len(train_data),
    )
...
def create_speechcommands35_classification_dataset(
    ...
    **kwargs,
    ...
    return Data(
        ...
        n_classes=N_CLASSES,
        in_dim=IN_DIM,
        train_pad_length=SEQ_LENGTH,
        test_pad_length=SEQ_LENGTH,
