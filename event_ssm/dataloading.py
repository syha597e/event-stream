...
def event_stream_collate_fn(batch, resolution, pad_unit, no_time_information=False):
    ...
    if len(resolution) == 1:
        ...
        tokens = [
            (
                e["x"][:-1] * e["y"][:-1]
                + np.prod(resolution) * e["p"][:-1].astype(np.int32)
            ).astype(np.int32)
            for e in x
        ]
    else:
    ...
    tokens = np.stack(
        [
            np.pad(e, (0, pad_length - len(e)), mode="constant", constant_values=-1)
            for e in tokens
        ]
    )
    timesteps = np.stack(
        [
            np.pad(e, (0, pad_length - len(e)), mode="constant", constant_values=0)
            for e in timesteps
        ]
    )
...
def event_stream_dataloader(
    ...
    val_loader = dataloader(
        val_data, eval_batch_size, eval_collate_fn, shuffle=False, drop_last=True
    )
    test_loader = dataloader(
        test_data, eval_batch_size, eval_collate_fn, shuffle=False, drop_last=False
    )
    ...
    transforms = tonic.transforms.Compose(
        [
            ...
            tonic.transforms.TimeJitter(
                std=time_jitter, clip_negative=False, sort_timestamps=True
            ),
    ...
    **kwargs,
    ...
    for batch in train_loader:
        ...
        if x.shape[1] == 0:
            ...
            exit()
    print("No sequence with zero seq. len")
    return train_loader, val_loader, test_loader, data
    ...
    **kwargs,
    ...
    train_transforms = [
        ...
        tonic.transforms.SpatialJitter(
            sensor_size=orig_sensor_size, var_x=1, var_y=1, clip_outliers=True
        ),
        (
